 ...
 *
 *    Description: This file is a set of functions for network
 * 
 *        Created:  2024.06.14
 *       Compiler:  gcc
 *
...
void cleanup_handler_close_skt(void *arg){
    int skt_fd = (int)arg;
    close(skt_fd);
    printf("socket closed\n");
}

void cleanup_handler_free_memory(void *arg){
    free(arg);
    printf("memory freed");
}
...
static void *_udp_server_create_and_start(void *arg){
    ...
    pthread_cleanup_push(cleanup_handler_close_skt, (void*)udp_sock_fd);
    pthread_cleanup_push(cleanup_handler_free_memory, (void*)recv_buffer);
